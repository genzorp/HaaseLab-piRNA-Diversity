...
source("/Users/genzorp/Documents/GITHUB/piDiversity/r/fromDaniel/simpleSRViolin_V5.R")
...
```{r EYE CANDY}
## SAMPLES
MILI.T1 <- F2.MOUSEDATA$ORIGINAL.DATA$MILI.PS.GRL$MILI_PS_1
#MILI.T1.BAM.PATH = "/Users/genzorp/Documents/GITHUB/piDiversity/data/PUBLIC/Zamore/bamFiles/SRR7760329.Aligned.sortedByCoord.out.bam"

MIWI.T1 <- F2.MOUSEDATA$ORIGINAL.DATA$MIWI.PS.GRL$MIWI_PS_1
#MIWI.T1.BAM.PATH = "/Users/genzorp/Documents/GITHUB/piDiversity/data/PUBLIC/Zamore/bamFiles/SRR7760307.Aligned.sortedByCoord.out.bam"

## CLEAN
rm(F2.MOUSEDATA)

## GET MOUSE CLUSTERS FROM THIS OBJECT
load("/Users/genzorp/Documents/GITHUB/piDiversity/sessions/F3_ALLDATA.RData")
ZAM.CLUST.GRS <- F3.ALLDATA$MOUSE.CLUSTERS
ZAM.WU.GRS <- F3.ALLDATA$MOUSE.WU
rm(F3.ALLDATA)

## ORDER
MILI.T1 <- MILI.T1[order(mcols(MILI.T1)[["MULT"]], decreasing = TRUE)]
MIWI.T1 <- MIWI.T1[order(mcols(MIWI.T1)[["MULT"]], decreasing = TRUE)]

## MIRBASE
source("/Users/genzorp/Documents/GITHUB/piDiversity/r/miRbase2BED.R")
MOUSE.MIR.PATH = "/Users/genzorp/Documents/DATA/Annotation/Mmusculus/mg38_mirBase_mmu.gff3"
MOUSE.MIR.GR <- makeGRangesFromDataFrame(df = miRbase2BED(miRBASEFILE = MOUSE.MIR.PATH), keep.extra.columns = TRUE)
MOUSE.MIR.GR

## REMOVE miRNAs
MILI.T1 <- subsetByOverlaps(x = MILI.T1, ranges = MOUSE.MIR.GR, type = "any", invert = TRUE)
MIWI.T1 <- subsetByOverlaps(x = MIWI.T1, ranges = MOUSE.MIR.GR, type = "any", invert = TRUE)

## SELECT TOP
MILI.T100 <- MILI.T1[1:100]
MIWI.T100 <- MIWI.T1[1:100]

## HOW MANY?
sum(mcols(MILI.T100)[["MULT"]])
sum(mcols(MIWI.T100)[["MULT"]])
...
## MULTIMAPPING LEVEL
MILI.T100.NH <- ggplot(data = as.data.table(MILI.T100)[,.N, by = "NH"]) + theme_pubclean() +
  geom_bar(aes(x = as.character(NH), y = N), stat = "identity") +
  ggtitle("Mappability of\nMILI Top 100 sequences") +
  ylab("% of TOP 100") + xlab("Mappability (NH tag)") +
  scale_y_continuous(limits = c(0,100)) +
  annotate(geom = "text", x = 1, y = 100, hjust = 0, colour = "firebrick3",
           label = paste0("# reads = ",sum(mcols(MILI.T100)[["MULT"]]))) +
  theme(aspect.ratio = 3)
MILI.T100.NH

MIWI.T100.NH <- ggplot(data = as.data.table(MIWI.T100)[,.N, by = "NH"]) + theme_pubclean() +
  geom_bar(aes(x = as.character(NH), y = N), stat = "identity") +
  ggtitle("Mappability of\nMIWI Top 100 sequences") +
  ylab("% of TOP 100") + xlab("Mappability (NH tag)") +
  scale_y_continuous(limits = c(0,100)) +
  annotate(geom = "text", x = 1, y = 100, hjust = 0, colour = "firebrick3",
           label = paste0("# reads = ",sum(mcols(MIWI.T100)[["MULT"]]))) +
  theme(aspect.ratio = 3)

ggarrange(plotlist = list(MILI.T100.NH, MIWI.T100.NH), ncol = 2)
...
MILI.T100.CHR <- ggplot(data = as.data.table(MILI.T100)[,.N, by = "seqnames"]) + theme_pubclean() +
  geom_bar(aes(x = seqnames, y = N), stat = "identity") +
  ggtitle("Genomic Position of MILI Top 100 seqeunces") +
  ylab("% of TOP 100") + xlab("") + 
  scale_y_continuous(limits = c(0,40)) +
  theme(aspect.ratio = 0.5, 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

MIWI.T100.CHR <- ggplot(data = as.data.table(MIWI.T100)[,.N, by = "seqnames"]) + theme_pubclean() +
  geom_bar(aes(x = seqnames, y = N), stat = "identity") +
  ggtitle("Genomic Position of MIWI Top 100 seqeunces") +
  ylab("% of TOP 100") + xlab("") + 
  scale_y_continuous(limits = c(0,40)) +
  theme(aspect.ratio = 0.5, 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

ggarrange(plotlist = list(MILI.T100.CHR, MIWI.T100.CHR), nrow = 2)

##
## OVERLAP WITH CLUSTERS?
## - not easy to answer since the clusters are based on mm9 genome
##

MILI.TOT.FMP <- F3.ALLDATA$MOUSE.FORCEMAPPED$`PSIMili-total`
MIWI.TOT.FMP <- F3.ALLDATA$MOUSE.FORCEMAPPED$`PSIMiwi-total`
rm(F3.ALLDATA)

## SIMPLIFY NAMES
LONG.MILI.L <- tstrsplit(names(MILI.TOT.FMP), split = "_")
SHOR.MILI.NAMES <- paste(LONG.MILI.L[[1]],LONG.MILI.L[[2]],LONG.MILI.L[[3]], sep = "_")
mcols(MILI.TOT.FMP)[["shortName"]] <- SHOR.MILI.NAMES
MILI.TOT.FMP

LONG.MIWI.L <- tstrsplit(names(MIWI.TOT.FMP), split = "_")
SHOR.MIWI.NAMES <- paste(LONG.MIWI.L[[1]],LONG.MIWI.L[[2]],LONG.MIWI.L[[3]], sep = "_")
mcols(MIWI.TOT.FMP)[["shortName"]] <- SHOR.MIWI.NAMES
MIWI.TOT.FMP

## SEARCH FOR TOP 100
MILI.T100.NAMES <- paste(seqnames(MILI.T100),start(MILI.T100),end(MILI.T100), sep = "_")
summary(mcols(MILI.TOT.FMP)[["shortName"]] %in% MILI.T100.NAMES)
MILI.FMP.T100 <- MILI.TOT.FMP[mcols(MILI.TOT.FMP)[["shortName"]] %in% MILI.T100.NAMES]
MILI.FMP.T100

MIWI.T100.NAMES <- paste(seqnames(MIWI.T100),start(MIWI.T100),end(MIWI.T100), sep = "_")
summary(mcols(MILI.TOT.FMP)[["shortName"]] %in% MILI.T100.NAMES)
MIWI.FMP.T100 <- MIWI.TOT.FMP[mcols(MIWI.TOT.FMP)[["shortName"]] %in% MIWI.T100.NAMES]
MIWI.FMP.T100

## BY CLUSTER MAPPING
MILI.FMP.T100.DOT <- ggplot(data = as.data.table(MILI.FMP.T100)[,.N, by = "seqnames"]) + theme_pubclean() +
  geom_point(aes(x = seqnames, y = N), stat = "identity", size = 5, colour = "slateblue1") +
  ggtitle("Forcemapped Cluster position of MILI Top 100 sequences") +
  ylab("% of TOP 100") + xlab("Zamore Cluster Name") + 
  scale_y_continuous(limits = c(0,16), breaks = seq(1,100,1)) +
  theme(aspect.ratio = 0.2, 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))

MIWI.FMP.T100.DOT <- ggplot(data = as.data.table(MIWI.FMP.T100)[,.N, by = "seqnames"]) + theme_pubclean() +
  geom_point(aes(x = seqnames, y = N), stat = "identity", size = 5, colour = "skyblue3") +
  ggtitle("Forcemapped Cluster position of MIWI Top 100 sequences") +
  ylab("% of TOP 100") + xlab("Zamore Cluster Name") + 
  scale_y_continuous(limits = c(0,16), breaks = seq(1,100,1)) +
  theme(aspect.ratio = 0.2, 
        axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
...
ggarrange(plotlist = list(MILI.FMP.T100.DOT, MIWI.FMP.T100.DOT), nrow = 2)
...
## METAGENE
source("/Users/genzorp/Documents/GITHUB/piDiversity/r/simpleMetageneV4.R")

META.MILI.T100 <- simpleMetagene4(GR = MILI.T100, 
                                  RANGE.NAME = "MILI.T100", 
                                  BSSPECIES = "Mmusculus",
...
simpleMetagene4(GR = MILI.FMP.T100,
                                  RANGE.NAME = "MILI.T100", 
                                  BSSPECIES = "Mmusculus",
                                  BSSPECIES.VERSION = "mm10", 
                                  EXPAND.BY = 50, 
...
META.MIWI.T100 <- simpleMetagene4(GR = MIWI.T100, 
                                  RANGE.NAME = "MIWI.T100", 
                                  BSSPECIES = "Mmusculus",
                                  BSSPECIES.VERSION = "mm10", 
                                  EXPAND.BY = 50, 
                                  USE.READS = TRUE, 
...
source("/Users/genzorp/Documents/GITHUB/piDiversity/r/simpleMetageneRegularGGV4.R")
MILI.META.GG <- simpleMetageneRegularGGV4(METAGENE.DT = META.MILI.T100[["frequency"]], 
                                          SAMPLE.NAME = "MILI.T100", ASPECT.RATIO = 2, Y.LIMITS = c(0,100))

MIWI.META.GG <- simpleMetageneRegularGGV4(METAGENE.DT = META.MIWI.T100[["frequency"]], 
                                          SAMPLE.NAME = "MIWI.T100", ASPECT.RATIO = 2, Y.LIMITS = c(0,100))

ggarrange(plotlist = list(MILI.META.GG,MIWI.META.GG), ncol = 2)
...
MILI.MAT <- as.matrix(META.MILI.T100[["count"]][,!"NAME"])
rownames(MILI.MAT) <- nth(tstrsplit(META.MILI.T100[["count"]][["NAME"]], split = "__"),-1)
L.MILI.T100 <- ggseqlogo(MILI.MAT) + 
  scale_x_continuous(limits = c(10,90), breaks = seq(1,100,1)) +
    ggtitle("MILI.T100 5`") +
...
MIWI.MAT <- as.matrix(META.MIWI.T100[["count"]][,!"NAME"])
rownames(MIWI.MAT) <- nth(tstrsplit(META.MIWI.T100[["count"]][["NAME"]], split = "__"),-1)
L.MIWI.T100 <- ggseqlogo(MIWI.MAT) + 
  scale_x_continuous(limits = c(10,90), breaks = seq(1,100,1)) +
  ggtitle("MIWI.T100 5`") +
  theme(aspect.ratio = 0.25,
        axis.text.x = element_text(angle = 90))
...
ggarrange(plotlist = list(L.MILI.T100,L.MIWI.T100), nrow = 2)
